...
public class MyBenchmarks
{
    ...
    public void Setup()
    {
        db = new Database();
        rooms = [
            db.Add(new Room() { name = "Room 1" }),
            db.Add(new Room() { name = "Room 2" }),
            db.Add(new Room() { name = "Room 3" }),
        ];
        for(var i=0; i<N; i++)
        {
            var row = db.Add(new Player() {userHash = i, position = new Vector2(i, i), roomId = rooms[i % rooms.Length].id});
        }
    }
    ...
    public void AddRow_WithUniqueIndex()
    {
        ...
        {
            var row = db.Add(new Player() {userHash = i});
        }
    }
    ...
    public void Iterate_RowContainer_1000Rows()
    {
        for(var x = 0; x < N; x++)
        {
            foreach (ref var i in db.PlayerTable.Rows())
            {
